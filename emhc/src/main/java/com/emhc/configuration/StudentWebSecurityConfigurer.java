...
public class StudentWebSecurityConfigurer extends WebSecurityConfigurerAdapter {
	...
	/*
	 * @Bean public ServiceProperties serviceProperties() { ServiceProperties sp
	 * = new ServiceProperties();
	 * sp.setService(env.getRequiredProperty(CAS_SERVICE_URL));
	 * sp.setSendRenew(false); return sp; }
	 */

	/*
	 * @Bean public RememberCasAuthenticationProvider
	 * casAuthenticationProvider() { RememberCasAuthenticationProvider
	 * casAuthenticationProvider = new RememberCasAuthenticationProvider();
	...
	/*
	 * @Bean public SessionAuthenticationStrategy sessionStrategy() {
	 * SessionFixationProtectionStrategy sessionStrategy = new
	...
	/*
	 * @Bean public Cas20ServiceTicketValidator cas20ServiceTicketValidator() {
	 * return new
	 * Cas20ServiceTicketValidator(env.getRequiredProperty(CAS_URL_PREFIX)); }
	 */
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		try {
			...
			http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/service/runlottery/**").permitAll()
					.antMatchers("/error/**").permitAll().antMatchers("/image/**").permitAll().antMatchers("/css/**")
					.permitAll().antMatchers("/student/template/**").permitAll().antMatchers("/student/login/**")
					.permitAll().antMatchers("/student/registration/**").permitAll().antMatchers("/student/**")
					.hasAuthority("CLIENT").and().formLogin().loginPage("/student/login")
					.successHandler(customSuccessHandler).failureUrl("/student/login?error")
					.usernameParameter("username").permitAll().and().logout().logoutUrl("/student/logout")
					.deleteCookies("remember-me").logoutSuccessUrl("/student/login").permitAll().and().rememberMe();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		// auth.userDetailsService(studentUserService).passwordEncoder(new
	...
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/i18n/**").antMatchers("/static/**");
	}
