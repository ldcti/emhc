	...
	public static class SecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			...
			http.authorizeRequests().antMatchers("/", "/student/login/**").permitAll()
					...
					.antMatchers("/student/login/**").permitAll().antMatchers("/student/**").hasAuthority("CLIENT")
					.anyRequest().authenticated().and()// .csrf().disable()
					.formLogin().loginPage("/student/login")
					// .successHandler(customSuccessHandler)
					.failureUrl("/login?error=true").usernameParameter("username").defaultSuccessUrl("/student/home")
					.permitAll().and().logout().logoutUrl("/student/logout").deleteCookies("remember-me")
					.logoutSuccessUrl("/student/login").permitAll().and().rememberMe();
		}
	}
	...
	public static class AdminSecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			...
			http.antMatcher("/admin/**").authorizeRequests().antMatchers("/admin/login/**").permitAll()
					...
					// .successHandler(customSuccessHandler)
					.usernameParameter("username").defaultSuccessUrl("/admin/home").permitAll().and().logout()
